...
| **Mass assignment, and unchecked input reaching a handler** | A route's [`input`](./validation/route-input) is enforced in Rust before any Lua runs: undeclared fields are stripped rather than passed through, text is coerced to the declared type, and every bound is checked. A failure is a `422` the handler never sees. Rule tables have a closed vocabulary, so a misspelt rule is a **load-time** error rather than a field nobody validates.                                                                                                                                                                                                                                                                                                                                                                                                           |
| **An upload that is not what it says it is**                | A validated `file` rule decides the type from the **bytes** — a signature beats the declared `Content-Type` — so a renamed executable does not pass as an image. Executables never match a `types` list without `allow_executables`; active content (`image/svg+xml`, `text/html`, `application/xhtml+xml`) never matches a family wildcard and logs a warning when named. `max_pixels` bounds a decompression bomb, `match_extension` stops a name disagreeing with its content, and the bytes stream to disk under `[multipart] upload_dir` without entering the Lua heap.                                                                                                                                                                                                                       |
...
- **A published API document is a map of your API.** Paths, parameter
  names, field names, bounds, which operations expect credentials. That
  is what it is for, and it is also reconnaissance for anyone probing
  you. Not a vulnerability — an API you can call is an API someone can
  enumerate — but it removes the work, which is why
  [`[openapi]` and `[swagger]`](./openapi/) are off by default and why
  publishing a [static site](./openapi/swagger-ui#a-static-site) from CI
  is a shape worth considering.
- **Response schemas are documentation, not enforcement.** A
  `doc.responses[…].schema` says what a handler intends to return;
  nothing checks that it does. Validating responses would turn a
  documentation mistake into a `500`, so the document marks them
  `x-nitr-enforced: false` and a test is what actually holds you to it.
...
validators, the text coercion that turns a query string into typed
values, the media-type sniffer that decides what an upload really is,
and the TLS certificate/key PEM the server loads at startup from
whatever an ACME client or a mounted secret wrote — eighteen targets in
all.
...
- [ ] Every request body, query string and path parameter declared as a
      route [`input`](./validation/route-input) — undeclared fields are
      stripped, which is what prevents mass assignment, and the check
      runs before any Lua does.
- [ ] Upload rules naming the types you accept, with `allow_executables`
      left off and `max_pixels` set on anything image-shaped.
- [ ] `[openapi] enabled` and `[swagger] enabled` decided deliberately:
      both are off by default, and `NITR_OPENAPI_ENABLED=false` is how
      you keep a route map out of production while keeping it in dev.
