...
own license — notably [Lua 5.4](https://www.lua.org/license.html) (MIT),
and, depending on the [Cargo features](./library/cargo-features)
enabled, SQLite (public domain), `reqwest`, `minijinja`, `argon2` and
the RustCrypto crates. `cargo license` or `cargo about` on a build of
your choosing produces the exact list for that build.
